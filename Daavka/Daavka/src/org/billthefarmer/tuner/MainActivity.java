...
public class MainActivity extends Activity {
	...
	private static final String	notes[]			= { "C", "C", "D", "E", "E",
			"F", "F", "G", "A", "A", "B", "B"	};
	...
	void getPreferences() {
		...
		if (audio != null) {
			audio.input = Integer.parseInt(preferences.getString(PREF_INPUT,
					"0"));
		...
		protected void processAudio() {
			...
			while (thread != null) {
				...
				for (int i = 0; i < STEP; i++) {
					...
					buffer[(SAMPLES - STEP) + i] = audio.filter ? yv[1]
							: data[i * divisor];
		...
		protected void copyToClipboard() {
			...
			if (multiple) {
				for (int i = 0; i < count; i++) {
					...
					text += String
							.format("%s%s%d\t%+5.2f\u00A2\t%4.2fHz\t%4.2fHz\t%+5.2fHz\n",
									notes[maxima.n[i] % OCTAVE],
									sharps[maxima.n[i] % OCTAVE], maxima.n[i]
											/ OCTAVE, cents, maxima.r[i],
									maxima.f[i], maxima.r[i] - maxima.f[i]);
				}
				...
				if (count == 0)
					text = String
							.format("%s%s%d\t%+5.2f\u00A2\t%4.2fHz\t%4.2fHz\t%+5.2fHz\n",
									notes[note % OCTAVE],
									sharps[note % OCTAVE], note / OCTAVE,
									cents, nearest, frequency, difference);
			}

			else
				text = String
						.format("%s%s%d\t%+5.2f\u00A2\t%4.2fHz\t%4.2fHz\t%+5.2fHz\n",
								notes[note % OCTAVE], sharps[note % OCTAVE],
								note / OCTAVE, cents, nearest, frequency,
								difference);

			ClipboardManager clipboard = (ClipboardManager) getSystemService(CLIPBOARD_SERVICE);

			// clipboard.setPrimaryClip(ClipData.newPlainText("Tuner clip",
			// text));
		}
	}
